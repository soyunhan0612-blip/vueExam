...
5. **AC는 실행 가능한 커맨드** — "~가 동작해야 한다" 같은 추상적 서술이 아닌 `npm run build` 같은 실제 실행 가능한 검증 커맨드를 포함한다. 이 프로젝트는 테스트 코드가 없으므로(PRD 비목표) `npm test`는 AC에 넣지 않는다.
...
! grep -rn "axios" src/components src/views   # 컴포넌트에서 axios 직접 호출 없음 (필요한 step만)
```
...
2. 아키텍처 체크리스트를 확인한다:
   ...
   - 이 step에서 만든 파일 중 PRD 5장 표에 있는 파일에 `[개념]` / `[Vue 2였다면]` 주석이 있는가?
